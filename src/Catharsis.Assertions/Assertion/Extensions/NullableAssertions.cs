...
public static class NullableAssertions
{
  ...
  extension(IAssertion assertion)
  {
    ...
    public IAssertion HasValue<T>(T? instance, string error = null) where T : struct => assertion.True(instance.HasValue, error);
    ...
    /// <exception cref="ArgumentNullException">If <paramref name="assertion"/> is <see langword="null"/>.</exception>
    /// <exception cref="InvalidOperationException">If the given <paramref name="assertion"/> is invalid.</exception>
    public IAssertion Value<T>(T? instance, T value, string error = null) where T : struct => assertion.Equal(instance.GetValueOrDefault(), value, error);
  }
}