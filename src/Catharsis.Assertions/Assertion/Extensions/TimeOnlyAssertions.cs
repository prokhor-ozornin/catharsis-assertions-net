...
public static class TimeOnlyAssertions
{
  ...
  extension(IAssertion assertion)
  {
    ...
    public IAssertion Hour(TimeOnly time, int hour, string error = null) => assertion.True(time.Hour == hour, error);
    ...
    /// <exception cref="ArgumentNullException">If <paramref name="assertion"/> is <see langword="null"/>.</exception>
    /// <exception cref="InvalidOperationException">If the given <paramref name="assertion"/> is invalid.</exception>
    public IAssertion Minute(TimeOnly time, int minute, string error = null) => assertion.True(time.Minute == minute, error);
    ...
    /// <exception cref="ArgumentNullException">If <paramref name="assertion"/> is <see langword="null"/>.</exception>
    /// <exception cref="InvalidOperationException">If the given <paramref name="assertion"/> is invalid.</exception>
    public IAssertion Second(TimeOnly time, int second, string error = null) => assertion.True(time.Second == second, error);
    ...
    /// <exception cref="ArgumentNullException">If <paramref name="assertion"/> is <see langword="null"/>.</exception>
    /// <exception cref="InvalidOperationException">If the given <paramref name="assertion"/> is invalid.</exception>
    public IAssertion Millisecond(TimeOnly time, int millisecond, string error = null) => assertion.True(time.Millisecond == millisecond, error);
  }
}
#endif