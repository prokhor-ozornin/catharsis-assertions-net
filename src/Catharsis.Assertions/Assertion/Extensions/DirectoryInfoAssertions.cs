...
public static class DirectoryInfoAssertions
{
  ...
  extension(IAssertion assertion)
  {
    ...
    public IAssertion Empty(DirectoryInfo directory, string error = null)
    {
    ...
    /// <exception cref="InvalidOperationException">If the given <paramref name="assertion"/> is invalid.</exception>
    public IAssertion InDirectory(DirectoryInfo directory, DirectoryInfo parent, string error = null)
    {
      if (assertion is null) throw new ArgumentNullException(nameof(assertion));
      if (directory is null) throw new ArgumentNullException(nameof(directory));
