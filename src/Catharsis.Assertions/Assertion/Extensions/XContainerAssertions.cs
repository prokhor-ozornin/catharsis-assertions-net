...
public static class XContainerAssertions
{
  ...
  extension(IAssertion assertion)
  {
    ...
    public IAssertion Element(XContainer container, XName name, string error = null)
    {
    ...
    /// <exception cref="InvalidOperationException">If the given <paramref name="assertion"/> is invalid.</exception>
    public IAssertion Empty(XContainer container, string error = null) => container is not null ? assertion.Empty(container.Nodes(), error) : throw new ArgumentNullException(nameof(container));
  }
}