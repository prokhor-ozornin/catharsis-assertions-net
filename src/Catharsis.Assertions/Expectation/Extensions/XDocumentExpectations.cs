...
public static class XDocumentExpectations
{
  ...
  extension(IExpectation<XDocument> expectation)
  {
    /// <summary>
    ...
    public IExpectation<XDocument> Empty() => expectation.HaveSubject().And().Expected(document => !document.Nodes().Any());
    ...
    /// <returns>Back self-reference to the given <paramref name="expectation"/>.</returns>
    /// <exception cref="ArgumentNullException">If the <paramref name="expectation"/> is either a <see langword="null"/> or has an undefined subject.</exception>
    public IExpectation<XDocument> Name(XName name) => expectation.HaveSubject().And().Expected(document => document.Root?.Name == name);
  }
}