...
public static class SecureStringExpectations
{
  ...
  extension(IExpectation<SecureString> expectation)
  {
    /// <summary>
    ...
    public IExpectation<SecureString> Length(int length) => expectation.HaveSubject().And().Expected(secure => secure.Length == length);
    ...
    /// <returns>Back self-reference to the given <paramref name="expectation"/>.</returns>
    /// <exception cref="ArgumentNullException">If the <paramref name="expectation"/> is either a <see langword="null"/> or has an undefined subject.</exception>
    public IExpectation<SecureString> Empty() => expectation.Length(0);
    ...
    /// <returns>Back self-reference to the given <paramref name="expectation"/>.</returns>
    /// <exception cref="ArgumentNullException">If the <paramref name="expectation"/> is either a <see langword="null"/> or has an undefined subject.</exception>
    public IExpectation<SecureString> ReadOnly() => expectation.HaveSubject().And().Expected(secure => secure.IsReadOnly());
  }
}