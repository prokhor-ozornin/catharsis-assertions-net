...
public static class HttpResponseMessageExpectations
{
  ...
  extension(IExpectation<HttpResponseMessage> expectation)
  {
    /// <summary>
    ...
    public IExpectation<HttpResponseMessage> Successful() => expectation.HaveSubject().And().Expected(response => response.IsSuccessStatusCode);
    ...
    /// <returns>Back self-reference to the given <paramref name="expectation"/>.</returns>
    /// <exception cref="ArgumentNullException">If the <paramref name="expectation"/> is either a <see langword="null"/> or has an undefined subject.</exception>
    public IExpectation<HttpResponseMessage> Status(HttpStatusCode status) => expectation.HaveSubject().And().Expected(response => response.StatusCode == status);
    ...
    public IExpectation<HttpResponseMessage> Header(string name, string value) => expectation.HaveSubject().And().ThrowIfNull(name, nameof(name)).And().Expected(response => response.Headers.Contains(name) && response.Headers.GetValues(name).Contains(value));
  }
}