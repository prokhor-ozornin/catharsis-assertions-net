...
/// </summary>
/// <seealso cref="RangeExpectations"/>
public sealed class RangeExpectationsTest : Test
{
  ...
  public void StartIndex_Method()
  {
    ...
    static void Test(bool result, Range range, int index) => range.Expect().StartIndex(index).Should().BeOfType<Expectation<Range>>().Which.Result.Should().Be(result);
  }
  ...
  public void EndIndex_Method()
  {
    ...
    static void Test(bool result, Range range, int index) => range.Expect().EndIndex(index).Should().BeOfType<Expectation<Range>>().Which.Result.Should().Be(result);
  }
}